...
controller.hears(['food', 'hungry', 'eat', 'restaurant'], ['direct_message', 'direct_mention', 'mention'], (bot, message) => {
  ...
  function askFood(convo) {
    convo.ask('What kind of food do you want?', (response) => {
      convo.say('Ok.');
  ...
  function askWhere(food, convo) {
    convo.ask('Where are you?', (response) => {
      convo.say(`Ok! I can find ${food.text} in ${response.text}. One moment.`);
      yelp.search({ term: `${food.text}`, location: `${response.text}` })
      .then((data) => {
        if (data.businesses.length === 0) {
          convo.say(`Sorry! I couldn't find ${food.text} in ${response.text}!`);
        } else {
